...
## 0. Stack & conventions
- **Language/runtime:** TypeScript on Node (matches the Claude Code/plugin ecosystem and lets the CLI shell out to `claude` cleanly). Single package, `@ccharness/core` + a thin `bin/` CLI in the same repo to start — split into packages only if it ever needs it.
...
- **Web UI (Milestone E):** React + Tailwind + recharts, served locally by `ccharness serve` over the same SQLite store via a thin read-only core API. No separate backend logic.
...
**Exit gate:** a one-page note confirming the catalog field mapping, the precise pi-pathfinder gap ccharness fills, and a provider decision (default + whether local JSON output is trustworthy enough). If pi-pathfinder turns out to already do deconflicted-stack reasoning against live inventory, stop and reassess scope before building.
...
3. Source config file (`~/.ccharness/marketplaces.yaml`): canonical catalog enabled by default; plus the official marketplace and any additional curated entries (pending PRD §12 Q1) for independence.
...
6. `ccharness sync` and `ccharness search`.
...
1. CLAUDE.md managed-block writer: locate/insert the `<!-- ccharness:start v… -->`…`<!-- ccharness:end -->` block; replace only its contents; bump embedded version.
...
**Goal:** `ccharness serve` launches a localhost dashboard that views the index, status, and recommendations — and changes nothing.
...
4. `ccharness serve [--port]`, binds localhost only.
...
**Exit gate:** every view renders from the live store; the Recommendation view's output for a given task matches `ccharness recommend "<task>"` exactly; there is no UI action that alters installed/enabled state or touches CLAUDE.md.
